...
Meteor.methods({
    ...
    qrcodeBulk: function(campaignId, QRbody, QRCount) {
        ...
        while ( i < QRCount) {
            ...
            var exists = Qrcodes.findOne({code: codeGen});
            if (exists){ 
                console.warn( 'Rejected. This qrcodes:'+codeGen+' already exists.' );
                if(j < 60)
                    continue;
                else
                    break;
    ...
    qrcodeImport: function(campaign,data) {
        ...
        console.log( 'id1:'+campaign._id);

        for ( let i = 0; i < data.length; i++ ) {
            let item   = data[ i ];
            let exists = Qrcodes.findOne({code: item.code});
